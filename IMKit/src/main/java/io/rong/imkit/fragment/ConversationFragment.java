...
public class ConversationFragment extends UriFragment implements OnScrollListener, IExtensionClickListener, IUserInfoProvider, EvaluateClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        this.mListAdapter.setMaxMessageSelectedCount(this.getResources().getInteger(integer.rc_max_message_selected_count));
        ...
        this.mList.setOnTouchListener(new OnTouchListener() {
            public boolean onTouch(View v, MotionEvent event) {
                if (event.getAction() == 2 && ConversationFragment.this.mList.getCount() - ConversationFragment.this.mList.getHeaderViewsCount() - ConversationFragment.this.mList.getFooterViewsCount() == 0) {
                    ...
                    if (event.getAction() == 0 && ConversationFragment.this.mRongExtension != null) {
        ...
        this.mListAdapter.setOnItemHandlerListener(new OnItemHandlerListener() {
            ...
            public void onMessageClick(int position, Message data, View v) {
                if (ConversationFragment.this.mRongExtension != null) {
                    ConversationFragment.this.mRongExtension.collapseExtension();
                }
    ...
    public void onResume() {
        ...
        if (this.isNeedRefresh && this.mListAdapter != null) {
    ...
    public List<IClickActions> getMoreClickActions() {
        ...
        if ((this.mConversationType == ConversationType.GROUP || this.mConversationType == ConversationType.PRIVATE) && RongContext.getInstance().getResources().getBoolean(bool.rc_enable_send_combine_message)) {
    ...
    protected void messageSelectedCountDidExceed() {
        Toast.makeText(this.getActivity(), string.rc_exceeded_max_limit_100, 0).show();
    }
        ...
        if (uri != null) {
            ...
            if (this.mConversationType.equals(ConversationType.CHATROOM)) {
                ...
                if (this.mConversationType.equals(ConversationType.CUSTOMER_SERVICE)) {
                    ...
                    RongMentionManager.getInstance().setAddMentionedMemberListener(new IAddMentionedMemberListener() {
                        public boolean onAddMentionedMember(UserInfo userInfo, int from) {
                            RLog.i("onAddMentionedMember", "from=" + from);
                            if (from == 0 && ConversationFragment.this.mRongExtension != null) {
                                ConversationFragment.this.mRongExtension.showSoftInput();
                            }
    ...
    public void onDestroy() {
        ...
        RecallEditManager.getInstance().cancelCountDownInConversation(this.mConversationType.getName() + this.mTargetId);
        super.onDestroy();
    }
    ...
    public boolean onBackPressed() {
        if (this.mRongExtension != null && this.mRongExtension.isExtensionExpanded()) {
            this.mRongExtension.collapseExtension();
            return true;
        } else if (this.mCustomServiceConfig != null && ConversationType.CUSTOMER_SERVICE.equals(this.mConversationType) && CSQuitSuspendType.NONE.equals(this.mCustomServiceConfig.quitSuspendType)) {
        ...
        while (var4.hasNext()) {
            ...
            switch (mediaType) {
                case 1:
                    ...
                    break;
                case 3:
                    SendMediaManager.getInstance().sendMedia(this.mConversationType, this.mTargetId, Collections.singletonList(Uri.parse(mediaUri)), origin, fireStatus, 10L);
                    if (this.mConversationType.equals(ConversationType.PRIVATE)) {
    ...
    public void onVoiceInputToggleTouch(View v, MotionEvent event) {
        ...
        if (!PermissionCheckUtil.checkPermissions(this.getActivity(), permissions) && event.getAction() == 0) {
            ...
            if (event.getAction() == 0) {
                ...
                if (IMLibExtensionModuleManager.getInstance().onRequestHardwareResource(ResourceType.VIDEO) || IMLibExtensionModuleManager.getInstance().onRequestHardwareResource(ResourceType.AUDIO)) {
                    Toast.makeText(this.getActivity(), this.getActivity().getString(string.rc_voip_occupying), 0).show();
                    return;
                }
    ...
    public Intent getSelectIntentForForward() {
        return new Intent(this.getActivity(), SelectConversationActivity.class);
    }

    private void forwardMessage(Intent data) {
        if (data != null) {
            ArrayList<Conversation> conversations = data.getParcelableArrayListExtra("conversations");
            ForwardManager.getInstance().forwardMessages(data.getIntExtra("index", 0), conversations, data.getIntegerArrayListExtra("messageIds"), this.getCheckedMessages());
            this.resetMoreActionState();
        }
    }
    ...
    private boolean isSelfSendMessage(Message msg) {
        String currentUserId = RongIM.getInstance().getCurrentUserId();
        String sendUserId = msg.getSenderUserId();
        if (currentUserId != null && sendUserId != null) {
            return currentUserId.equals(sendUserId) && this.mTargetId.equals(msg.getTargetId());
        } else {
            return false;
        }
    }
    ...
    private void showNewMessage() {
        if (RongContext.getInstance().getNewMessageState() && this.mNewMessageCount >= 0) {
            if (this.mNewMessageCount == 0) {
                ...
                this.mNewMessageTextView.setText(this.mNewMessageCount > 99 ? "99+" : Integer.toString(this.mNewMessageCount));
            }
        }
    }
    ...
    public void onEventMainThread(OnReceiveMessageEvent event) {
        ...
        if (this.mConversationType.equals(conversationType) && this.mTargetId.equals(targetId) && this.shouldUpdateMessage(message, event.getLeft())) {
            if (event.getLeft() == 0) {
                if (!event.hasPackage()) {
                    HQVoiceMsgDownloadManager.getInstance().enqueue(this, new AutoDownloadEntry(message, DownloadPriority.HIGH));
                }
    ...
    protected void increaseNewMessageCountIfNeed(Message message) {
        this.updateNewMessageCountIfNeed(message, true);
    }

    protected void updateNewMessageCountIfNeed(Message message, boolean increase) {
        if (this.mNewMessageBtn != null && this.getContext() != null) {
    ...
    public void onEventMainThread(OnReceiveMessageProgressEvent event) {
        if (this.mList != null) {
            boolean isInBackground = SystemUtils.isInBackground(this.getContext());
    ...
    public void onEventMainThread(ShowDurnDialogEvent event) {
        (new BurnHintDialog()).show(this.getFragmentManager());
    }

    public void onEventMainThread(RecallMessageEditClickEvent event) {
        Message message = event.getMessage();
    ...
    public void getHistoryMessage(final ConversationType conversationType, final String targetId, int lastMessageId, int reqCount, ConversationFragment.LoadMessageDirection direction, final IHistoryDataResultCallback<List<Message>> callback) {
        if (direction == ConversationFragment.LoadMessageDirection.UP) {
            ...
            RongIMClient.getInstance().getHistoryMessages(conversationType, targetId, this.indexMessageTime, before, after, new ResultCallback<List<Message>>() {
                public void onSuccess(List<Message> messages) {
                    if (callback != null) {
                        callback.onResult(ConversationFragment.this.filterDestructionMessage(conversationType, targetId, messages));
                    }
            ...
            while (true) {
                while (var6.hasNext()) {
                    ...
                    if (message.getContent().isDestruct() && message.getReadTime() > 0L) {
                        ...
                        long delay = (System.currentTimeMillis() - RongIMClient.getInstance().getDeltaTime() - message.getReadTime()) / 1000L;
    ...
    private void getHistoryMessage(ConversationType conversationType, String targetId, int reqCount, Mode mode, final int scrollMode, int messageId) {
        this.mList.onRefreshStart(mode);
        final int getHistoryCount;
        int count;
        if (conversationType.equals(ConversationType.CHATROOM)) {
            count = this.getResources().getInteger(integer.rc_chatroom_first_pull_message_count);
            if (count == 0) {
                getHistoryCount = 10;
            } else {
                if (count == -1) {
        ...
        this.getHistoryMessage(conversationType, targetId, count, getHistoryCount, direction, new IHistoryDataResultCallback<List<Message>>() {
            public void onResult(List<Message> messages) {
                ...
                if (direction == ConversationFragment.LoadMessageDirection.DOWN) {
                    ...
                    ConversationFragment.this.mList.onRefreshComplete(msgCount, getHistoryCount, false);
                    ConversationFragment.this.mHasMoreLocalMessagesUp = msgCount == getHistoryCount;
                }
                ...
                if (messages != null && messages.size() > 0) {
                    ...
                    ConversationFragment.this.refreshUI(messages, needRefresh, scrollMode, finalCount, direction);
            ...
            public void onError() {
                ConversationFragment.this.mList.onRefreshComplete(getHistoryCount, getHistoryCount, false);
            }
        });
    }
    ...
    private void refreshUI(List<Message> messages, boolean needRefresh, int scrollMode, int last, ConversationFragment.LoadMessageDirection direction) {
        if (needRefresh) {
            this.mListAdapter.notifyDataSetChanged();
            int selected;
    ...
    private void getRemoteHistoryMessages(ConversationType conversationType, String targetId, int reqCount) {
        this.mList.onRefreshStart(Mode.START);
        final int getHistoryCount;
        if (conversationType.equals(ConversationType.CHATROOM)) {
            int count = this.getResources().getInteger(integer.rc_chatroom_first_pull_message_count);
            if (count == 0) {
                getHistoryCount = 10;
            } else {
                if (count == -1) {
            ...
            public void onResult(List<Message> messages) {
                ...
                if (messages != null && messages.size() > 0) {
                    ...
                    if (remoteList != null && remoteList.size() > 0) {
                        ...
                        ConversationFragment.this.mList.onRefreshComplete(messages.size(), getHistoryCount, false);
                        if (lastMessage != null) {
                            RongContext.getInstance().getEventBus().post(lastMessage);
                        }
                    } else {
                        ConversationFragment.this.mList.onRefreshComplete(0, getHistoryCount, false);
                    }
                } else {
                    ConversationFragment.this.mList.onRefreshComplete(0, getHistoryCount, false);
                }
    ...
    public void onExtensionExpanded(int h) {
        if (this.mRongExtension.getTriggerMode() == 2) {
