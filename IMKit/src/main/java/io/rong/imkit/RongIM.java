...
import io.rong.imlib.RongIMClient.EncryptedSessionConnectionListener;
...
import io.rong.imlib.RongIMClient.OnReceiveDestructionMessageListener;
...
public class RongIM {
    ...
    private List<EncryptedSessionConnectionListener> mEncSessionListeners;
    ...
    private RongIM.SamplingRate sampleRate;
    private RongIM.VoiceMessageType voiceMessageType;

    private RongIM() {
        this.sampleRate = RongIM.SamplingRate.RC_SAMPLE_RATE_8000;
        this.voiceMessageType = RongIM.VoiceMessageType.Ordinary;
        this.mClientWrapper = new RongIMClientWrapper();
        this.mEncSessionListeners = new LinkedList();
        HandlerThread workThread = new HandlerThread("KIT_WORK");
        workThread.start();
        this.mWorkHandler = new Handler(workThread.getLooper());
        RongIMClient.getInstance().setEncryptedSessionConnectionListener(new EncryptedSessionConnectionListener() {
            public void onEncryptedSessionRequest(String targetId, boolean isSuccess) {
                RLog.d(RongIM.TAG, "发送加密会话请求");
                if (RongIM.this.mEncSessionListeners != null) {
                    for (int i = 0; i < RongIM.this.mEncSessionListeners.size(); ++i) {
                        if (RongIM.this.mEncSessionListeners.get(i) != null) {
                            ((EncryptedSessionConnectionListener) RongIM.this.mEncSessionListeners.get(i)).onEncryptedSessionRequest(targetId, isSuccess);
                        }
                    }
                }
            ...
            public void onEncryptedSessionResponse(String targetId) {
                RLog.d(RongIM.TAG, "发送加密会话响应");
                if (RongIM.this.mEncSessionListeners != null) {
                    for (int i = 0; i < RongIM.this.mEncSessionListeners.size(); ++i) {
                        if (RongIM.this.mEncSessionListeners.get(i) != null) {
                            ((EncryptedSessionConnectionListener) RongIM.this.mEncSessionListeners.get(i)).onEncryptedSessionResponse(targetId);
                        }
                    }
                }
            ...
            public void onEncryptedSessionEstablished(String targetId) {
                RLog.d(RongIM.TAG, "加密会话建立");
                if (targetId != null && targetId.length() != 0) {
                    if (RongIM.this.mEncSessionListeners != null) {
                        for (int i = 0; i < RongIM.this.mEncSessionListeners.size(); ++i) {
                            if (RongIM.this.mEncSessionListeners.get(i) != null) {
                                ((EncryptedSessionConnectionListener) RongIM.this.mEncSessionListeners.get(i)).onEncryptedSessionEstablished(targetId);
                            }
                        }
                    }

                } else {
                    RLog.e(RongIM.TAG, "encrypted session establish failed because the targetId is null or empty.");
                }
            }

            public void onEncryptedSessionCanceled(String targetId) {
                RLog.d(RongIM.TAG, "取消加密会话请求");
                if (RongIM.this.mEncSessionListeners != null) {
                    for (int i = 0; i < RongIM.this.mEncSessionListeners.size(); ++i) {
                        if (RongIM.this.mEncSessionListeners.get(i) != null) {
                            ((EncryptedSessionConnectionListener) RongIM.this.mEncSessionListeners.get(i)).onEncryptedSessionCanceled(targetId);
                        }
                    }
                }
            ...
            public void onEncryptedSessionTerminated(String targetId) {
                RLog.d(RongIM.TAG, "结束加密会话请求");
                if (RongIM.this.mEncSessionListeners != null) {
                    for (int i = 0; i < RongIM.this.mEncSessionListeners.size(); ++i) {
                        if (RongIM.this.mEncSessionListeners.get(i) != null) {
                            ((EncryptedSessionConnectionListener) RongIM.this.mEncSessionListeners.get(i)).onEncryptedSessionTerminated(targetId);
                        }
                    }
                }
    ...
    public void registerEncSessionListener(EncryptedSessionConnectionListener listener) {
    ...
    public void unRegisterEncSessionListener(EncryptedSessionConnectionListener listener) {
        if (listener != null) {
    ...
    public boolean isRegisted(EncryptedSessionConnectionListener listener) {
    ...
    public static void init(Application application, String appKey) {
        RongIM.SingletonHolder.sRongIM.initSDK(application, appKey, true);
    }

    public static void init(Context context, String appKey) {
        RongIM.SingletonHolder.sRongIM.initSDK(context, appKey, true);
    }

    public static void init(Context context, String appKey, boolean enablePush) {
        RongIM.SingletonHolder.sRongIM.initSDK(context, appKey, enablePush);
    }

    public static void init(Context context) {
        RongIM.SingletonHolder.sRongIM.initSDK(context, (String) null, true);
    }

    public static void initAsync(final Context context, final String appKey, final boolean enablePush, final RongIM.AsyncInitListener asyncInitListener) {
        RongIM.SingletonHolder.sRongIM.mWorkHandler.post(new Runnable() {
            public void run() {
    ...
    public void setSavePath(String savePath) {
        ...
        RongIMClient.getInstance().setSavePath(savePath);
    }

    public static RongIM connect(String token, final ConnectCallback callback) {
        return connect(token, new ConnectCallbackEx() {
    ...
    public static RongIM connect(final String token, final ConnectCallbackEx connectCallbackEx) {
        RongIMClient.connect(token, new ConnectCallbackEx() {
            public void onSuccess(String userId) {
                RongIM.saveToken(token);
                if (connectCallbackEx != null) {
                    connectCallbackEx.onSuccess(userId);
                }
            ...
            public void onError(ErrorCode e) {
                if (connectCallbackEx != null) {
                    connectCallbackEx.onError(e);
                }

                RongExtensionManager.getInstance().connect(token);
                InternalModuleManager.getInstance().onConnected(token);
                if (RongContext.getInstance() == null) {
                    RLog.d(RongIM.TAG, "connect - onError. RongIM SDK not init, ConnectEvent do not post.");
                } else {
            ...
            public void OnDatabaseOpened(DatabaseOpenStatus code) {
                if (connectCallbackEx != null) {
                    connectCallbackEx.OnDatabaseOpened(code);
                }
    ...
    private static void initListener() {
        RongIMClient.setOnReceiveMessageListener(new OnReceiveMessageWrapperListener() {
            public boolean onReceived(Message message, int left, boolean hasPackage, boolean offline) {
                ...
                if (RongIM.sMessageListener != null) {
                    if (RongIM.sMessageListener instanceof OnReceiveMessageWrapperListener) {
                        isProcess = ((OnReceiveMessageWrapperListener) RongIM.sMessageListener).onReceived(message, left, hasPackage, offline);
                    } else {
                ...
                if (msgTag != null && (msgTag.flag() == 3 || msgTag.flag() == 1)) {
                    ...
                    RongContext.getInstance().getEventBus().post(new OnReceiveMessageEvent(message, left, hasPackage, offline));
        ...
        try {
            readRec = RongContext.getInstance().getResources().getBoolean(bool.rc_read_receipt);
        ...
        try {
            syncReadStatus = RongContext.getInstance().getResources().getBoolean(bool.rc_enable_sync_read_status);
        ...
        RongIMClient.setOnRecallMessageListener(new OnRecallMessageListener() {
            public boolean onMessageRecalled(Message message, RecallNotificationMessage recallNotificationMessage) {
                ...
                if (recallNotificationMessage == null) {
                    RLog.i(RongIM.TAG, "Delete the recall message, recallNotificationMessage is null");
                    return true;
                } else {
        ...
        RongIMClient.getInstance().setOnReceiveDestructionMessageListener(new OnReceiveDestructionMessageListener() {
            public void onReceive(Message message) {
                EventBus.getDefault().post(new MessageDeleteEvent(new int[]{message.getMessageId()}));
            }
        });
    ...
    public void getUnreadCount(ConversationType[] conversationTypes, boolean containBlocked, ResultCallback<Integer> callback) {
    ...
    public void deleteRemoteMessages(ConversationType conversationType, String targetId, final Message[] messages, final OperationCallback callback) {
        RongIMClient.getInstance().deleteRemoteMessages(conversationType, targetId, messages, new OperationCallback() {
        ...
        if (tag != null && (tag.flag() & 1) == 1) {
            RongIMClient.getInstance().insertMessage(type, targetId, senderUserId, content, sentTime, new ResultCallback<Message>() {
                ...
                public void onError(ErrorCode e) {
                    ...
                    if (RongContext.getInstance() == null) {
                        RLog.e(RongIM.TAG, "insertMessage. RongIM SDK not init, please do after init.");
                    } else {
    ...
    public Message insertMessage(ConversationType type, String targetId, String senderUserId, MessageContent content) {
        ...
        if (RongContext.getInstance() == null) {
            RLog.e(TAG, "insertMessage. RongIM SDK not init, please do after init.");
        } else {
            RongContext.getInstance().getEventBus().post(message);
        }
        ...
        if (tag != null && (tag.flag() & 1) == 1) {
            RongIMClient.getInstance().insertIncomingMessage(type, targetId, senderUserId, receivedStatus, content, sentTime, new ResultCallback<Message>() {
                ...
                public void onError(ErrorCode e) {
                    ...
                    if (RongContext.getInstance() == null) {
                        RLog.e(RongIM.TAG, "insertIncomingMessage. RongIM SDK not init, please do after init.");
                    } else {
                        RongContext.getInstance().getEventBus().post(e);
                    }
        ...
        if (tag != null && (tag.flag() & 1) == 1) {
            RongIMClient.getInstance().insertOutgoingMessage(type, targetId, sentStatus, content, sentTime, new ResultCallback<Message>() {
                ...
                public void onError(ErrorCode e) {
                    ...
                    if (RongContext.getInstance() == null) {
                        RLog.e(RongIM.TAG, "insertOutgoingMessage. RongIM SDK not init, please do after init.");
                    } else {
                        RongContext.getInstance().getEventBus().post(e);
                    }
            ...
            Message message = RongIMClient.getInstance().sendMessage(type, targetId, content, pushContent, pushData, new SendMessageCallback() {
                public void onSuccess(Integer messageId) {
                    if (result.t != null) {
                        ...
                        RongIM.this.filterSentMessage((Message) result.t, (ErrorCode) null);
                        if (callback != null) {
                ...
                public void onError(Integer messageId, ErrorCode errorCode) {
                    if (result.t != null) {
                        if (!errorCode.equals(ErrorCode.RC_MSG_REPLACED_SENSITIVE_WORD)) {
    ...
    public void sendMessage(ConversationType type, String targetId, MessageContent content, String pushContent, String pushData, final SendMessageCallback callback, final ResultCallback<Message> resultCallback) {
        ...
        if (temp != null) {
            ...
            RongIMClient.getInstance().sendMessage(type, targetId, content, pushContent, pushData, new SendMessageCallback() {
                ...
                public void onError(ErrorCode e) {
                    if (RongContext.getInstance() == null) {
                        RLog.e(RongIM.TAG, "sendMessage. RongIM SDK not init, please do after init.");
                    } else {
                        RongContext.getInstance().getEventBus().post(e);
                    }
    ...
    public void sendMessage(Message message, String pushContent, String pushData, final ISendMessageCallback callback) {
        ...
        if (filterMsg == null) {
            ...
            RongIMClient.getInstance().sendMessage(message, pushContent, pushData, new ISendMessageCallback() {
                ...
                public void onSuccess(Message message) {
                    RongIM.this.filterSentMessage(message, (ErrorCode) null);
                    if (callback != null) {
                        callback.onSuccess(message);
                    }
    ...
    public void sendMessage(Message message, String pushContent, String pushData, SendMessageOption option, final ISendMessageCallback callback) {
        Message filterMsg = this.filterSendMessage(message);
        if (filterMsg == null) {
            RLog.w(TAG, "sendMessage: 因在 onSend 中消息被过滤为 null，取消发送。");
        } else {
            ...
            message.setContent(this.setMessageAttachedUserInfo(message.getContent()));
            RongIMClient.getInstance().sendMessage(message, pushContent, pushData, option, new ISendMessageCallback() {
                public void onAttached(Message message) {
                    MessageTag tag = (MessageTag) message.getContent().getClass().getAnnotation(MessageTag.class);
                    if (tag != null && (tag.flag() & 1) == 1) {
                        RongContext.getInstance().getEventBus().post(message);
    ...
    public void downloadMediaMessage(Message message, final IDownloadMediaMessageCallback callback) {
        RongIMClient.getInstance().downloadMediaMessage(message, new IDownloadMediaMessageCallback() {
            public void onSuccess(Message message) {
                OnReceiveMessageProgressEvent result = new OnReceiveMessageProgressEvent();
    ...
    private void filterSentMessage(Message message, ErrorCode errorCode) {
        ...
        MessageTag tag;
        if (errorCode != null && !isExecute && errorCode != ErrorCode.RC_MSG_REPLACED_SENSITIVE_WORD) {
            if (errorCode.equals(ErrorCode.NOT_IN_DISCUSSION) || errorCode.equals(ErrorCode.NOT_IN_GROUP) || errorCode.equals(ErrorCode.NOT_IN_CHATROOM) || errorCode.equals(ErrorCode.REJECTED_BY_BLACKLIST) || errorCode.equals(ErrorCode.FORBIDDEN_IN_GROUP) || errorCode.equals(ErrorCode.FORBIDDEN_IN_CHATROOM) || errorCode.equals(ErrorCode.KICKED_FROM_CHATROOM)) {
                ...
                if (errorCode.equals(ErrorCode.NOT_IN_DISCUSSION)) {
                    informationMessage = InformationNotificationMessage.obtain(mContext.getString(string.rc_info_not_in_discussion));
                } else if (errorCode.equals(ErrorCode.NOT_IN_GROUP)) {
                    informationMessage = InformationNotificationMessage.obtain(mContext.getString(string.rc_info_not_in_group));
                } else if (errorCode.equals(ErrorCode.NOT_IN_CHATROOM)) {
                    informationMessage = InformationNotificationMessage.obtain(mContext.getString(string.rc_info_not_in_chatroom));
                } else if (errorCode.equals(ErrorCode.REJECTED_BY_BLACKLIST)) {
                    informationMessage = InformationNotificationMessage.obtain(mContext.getString(string.rc_rejected_by_blacklist_prompt));
                } else if (errorCode.equals(ErrorCode.FORBIDDEN_IN_GROUP)) {
                    informationMessage = InformationNotificationMessage.obtain(mContext.getString(string.rc_info_forbidden_to_talk));
                } else if (errorCode.equals(ErrorCode.FORBIDDEN_IN_CHATROOM)) {
                    informationMessage = InformationNotificationMessage.obtain(mContext.getString(string.rc_forbidden_in_chatroom));
                } else if (errorCode.equals(ErrorCode.KICKED_FROM_CHATROOM)) {
                    informationMessage = InformationNotificationMessage.obtain(mContext.getString(string.rc_kicked_from_chatroom));
                }
            ...
            if (RongContext.getInstance() != null && tag != null && (tag.flag() & 1) == 1) {
                RongContext.getInstance().getEventBus().post(new OnMessageSendErrorEvent(message, errorCode));
            }
        } else if (message != null) {
            content = message.getContent();
            tag = (MessageTag) content.getClass().getAnnotation(MessageTag.class);
            if (RongContext.getInstance() != null && tag != null && (tag.flag() & 1) == 1 && message.getSentStatus().getValue() != SentStatus.CANCELED.getValue()) {
                RongContext.getInstance().getEventBus().post(message);
            }
        }
