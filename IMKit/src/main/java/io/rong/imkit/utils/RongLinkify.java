...
public class RongLinkify {
  ...
  public static final boolean addLinks(Spannable text, int mask) {
    if (mask == 0) {
      ...
      ArrayList<RongLinkify.LinkSpec> links = new ArrayList();
      if ((mask & 1) != 0) {
      ...
      if (links.size() == 0) {
        ...
        while(var4.hasNext()) {
          RongLinkify.LinkSpec link = (RongLinkify.LinkSpec)var4.next();
  ...
  private static final void gatherLinks(ArrayList<RongLinkify.LinkSpec> links, Spannable s, Pattern pattern, String[] schemes, android.text.util.Linkify.MatchFilter matchFilter, android.text.util.Linkify.TransformFilter transformFilter) {
    ...
    while(true) {
      ...
      RongLinkify.LinkSpec spec = new RongLinkify.LinkSpec();
  ...
  private static final void gatherTelLinks(ArrayList<RongLinkify.LinkSpec> links, Spannable s, Pattern pattern) {
    Matcher m = pattern.matcher(s);
  ...
  private static final void gatherMapLinks(ArrayList<RongLinkify.LinkSpec> links, Spannable s) {
  ...
  private static final void pruneOverlaps(ArrayList<RongLinkify.LinkSpec> links) {
    Comparator<RongLinkify.LinkSpec> c = new Comparator<RongLinkify.LinkSpec>() {
      public final int compare(RongLinkify.LinkSpec a, RongLinkify.LinkSpec b) {
        if (a.start < b.start) {
          return -1;
    ...
    while(true) {
      while(i < len - 1) {
        RongLinkify.LinkSpec a = (RongLinkify.LinkSpec)links.get(i);
        RongLinkify.LinkSpec b = (RongLinkify.LinkSpec)links.get(i + 1);
        int remove = -1;
