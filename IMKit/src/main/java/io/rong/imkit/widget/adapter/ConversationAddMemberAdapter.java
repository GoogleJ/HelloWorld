...
public class ConversationAddMemberAdapter extends BaseAdapter<UserInfo> {
  ...
  private ConversationAddMemberAdapter.OnDeleteIconListener mDeleteIconListener;
  ...
  protected View newView(Context context, int position, ViewGroup group) {
    View result = this.mInflater.inflate(layout.rc_item_conversation_member, (ViewGroup)null);
    ConversationAddMemberAdapter.ViewHolder holder = new ConversationAddMemberAdapter.ViewHolder();
    holder.mMemberIcon = (AsyncImageView)this.findViewById(result, 16908294);
  ...
  protected void bindView(View v, final int position, UserInfo data) {
    ConversationAddMemberAdapter.ViewHolder holder = (ConversationAddMemberAdapter.ViewHolder)v.getTag();
    if (!data.getUserId().equals("RongAddBtn") && !data.getUserId().equals("RongDelBtn")) {
      ...
      if (data.getPortraitUri() != null) {
        holder.mMemberIcon.setResource(data.getPortraitUri().toString(), drawable.rc_default_portrait);
      }
      ...
      if (data.getUserId().equals("RongAddBtn")) {
        holder.mMemberDeIcon.setImageResource(drawable.rc_ic_setting_friends_add);
      } else {
        holder.mMemberDeIcon.setImageResource(drawable.rc_ic_setting_friends_delete);
      }
  ...
  public void setDeleteIconListener(ConversationAddMemberAdapter.OnDeleteIconListener listener) {
